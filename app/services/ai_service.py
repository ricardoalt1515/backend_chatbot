...
class AIService:
    ...
    def __init__(self):
        ...
        if self.provider == "groq":
            if not settings.GROQ_API_KEY:
                logger.warning("GROQ_API_KEY no configurada. Las llamadas a la API fallarán.")
            ...
            if not settings.OPENAI_API_KEY:
                logger.warning("OPENAI_API_KEY no configurada. Las llamadas a la API fallarán.")
            ...
            logger.warning(f"Proveedor de IA no soportado: {self.provider}. Usando respuestas pre-configuradas.")
            self.api_key = None
            self.model = None
            self.api_url = None
    ...
    def _get_fallback_response(self, messages: List[Dict[str, Any]]) -> str:
        ...
        if "hola" in last_user_message or "saludos" in last_user_message or not last_user_message:
            return ("¡Hola! Soy el asistente virtual de Hydrous especializado en soluciones de reciclaje de agua. "
                   "¿En qué puedo ayudarte hoy?")
        
        elif any(word in last_user_message for word in ["filtro", "filtración", "purificación"]):
            return ("Nuestros sistemas de filtración avanzada eliminan contaminantes, sedimentos y patógenos "
        ...
        elif any(word in last_user_message for word in ["aguas grises", "duchas", "lavadora"]):
            return ("Nuestros sistemas de tratamiento de aguas grises reciclan el agua de duchas, lavabos y lavadoras "
        ...
        elif any(word in last_user_message for word in ["lluvia", "pluvial", "captación"]):
            return ("Los sistemas de captación de agua de lluvia de Hydrous incluyen filtración, almacenamiento "
        ...
        elif "precio" in last_user_message or "costo" in last_user_message or "valor" in last_user_message:
            return ("Los precios varían según el tipo de sistema y las necesidades específicas de tu proyecto. "
            ...
            return ("Gracias por tu pregunta. Para brindarte la información más precisa sobre nuestras "
    ...
    async def handle_conversation(self, conversation: Conversation, user_message: str) -> str:
        ...
        if self._should_start_questionnaire(user_message):
            conversation.start_questionnaire()
            intro_text, explanation = questionnaire_service.get_introduction()
            next_question = questionnaire_service.get_next_question(conversation.questionnaire_state)
    ...
    async def _handle_questionnaire_flow(self, conversation: Conversation, user_message: str) -> str:
        """
        ...
        if previous_question_id:
            questionnaire_service.process_answer(conversation, previous_question_id, user_message)
...
ai_service = AIWithQuestionnaireService()
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    logger.error(f"Error en la API de {self.provider}: {response.status_code} - {response.text}")
                    return self._get_fallback_response(messages)
